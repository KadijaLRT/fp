  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50">
      <div className="bg-slate-800 w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl p-5">
        <h2 className="text-lg font-bold text-slate-50 mb-1">Need to be done by a certain time?</h2>
        ...
        <input
          ...
          autoFocus
          className="w-full h-14 px-3 rounded-xl bg-slate-900 border border-slate-700 text-slate-100 text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-amber-400 mb-3"
        />

        {error && <p role="alert" className="text-xs text-red-400 font-semibold mb-3">{error}</p>}

        <div className="grid grid-cols-2 gap-2">
          <button onClick={onSkip} className="py-3 rounded-xl bg-slate-700 text-slate-300 font-semibold min-h-[48px]">
            Skip
          </button>
          <button onClick={handleSave} className="py-3 rounded-xl bg-amber-500 text-slate-900 font-bold min-h-[48px]">
            Save
          </button>
        </div>
      </div>
